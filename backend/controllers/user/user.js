...
async function createUser(req, res) {
  try {
    ...
    const token = jwt.sign({ userId: user.id }, "soffie"); // Ganti 'soffie' dengan kunci rahasia Anda
  ...
  try {
    ...
    if (userKasir.role == "kasir") {
      ...
      const token = jwt.sign(
        ...
        "soffie",
      ); // Ganti 'soffie' dengan kunci rahasia Anda
...
function verifyToken(req, res, next) {
  ...
  jwt.verify(token, "soffie", (error, decoded) => {
    if (error) {
